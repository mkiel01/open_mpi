...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < MESSAGE_SIZES_COUNT; i++) {
        ...
        double total_time_synchronous = 0;
        
        char *buffer = (char *)malloc(message_size * sizeof(char));
        
        // Synchronous Communication with MPI_Ssend
        for (int j = 0; j < REPETITIONS; j++) {
            ...
            if (rank == 0) {
                ...
                MPI_Ssend(buffer, message_size, MPI_CHAR, 0, 0, MPI_COMM_WORLD);
            }
            end_time = MPI_Wtime();
            total_time_synchronous += (end_time - start_time);
        }
        
        double average_latency_synchronous = total_time_synchronous / (2 * REPETITIONS); // Two processes
        double total_bytes_transferred = 2 * REPETITIONS * message_size;
        double throughput_mbps = (total_bytes_transferred * BITS_PER_BYTE) / (total_time_synchronous * MEGABIT);
        
        printf("Total Message size: %f - Total time taken: %f seconds\n", total_bytes_transferred, total_time_synchronous);
        printf("Message size: %d: Synchronous Communication - Average latency: %f seconds\n", message_size, average_latency_synchronous);
