...
int main(int argc, char *argv[]) {
    ...
    srand(rank * time(NULL)); // TO JEST BARDZO WAZNE ZE MNOZYMY TUTAJ ZAMIAST DODAWAC

    start = MPI_Wtime(); // Rozpoczęcie pomiaru czasu

    for (long j = 0; j < num; j++) {
        // Generowanie dwóch liczb losowych w zakresie [0, 1]
        xcoordinate = (double)rand() / RAND_MAX; // sprwadzic czy jest na pewno git te randi!!!sranda mam i jest git bo ma byc wiec chyba git 
    ...
    MPI_Barrier(MPI_COMM_WORLD); // niepotrzebne w tym miejscu można usunąć to
